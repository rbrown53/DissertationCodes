...
for i=1:nparams
    ...
    title([append('$',names(i),'$')],'interpreter','latex')%,':  IACT=',num2str(taux(i)),'  p_{Geweke}=',num2str(px(i))])
end

% Individual histograms.
figure(k+1) 
for i=1:nparams
    subplot(nparams,1,i),histogram(x(i,:),20,'FaceAlpha',1), colormap(1-gray)
    title([append('$',names(i),'$')],'interpreter','latex')%,'\tau_{int}=',num2str(taux(i)),' and  p_{Geweke}=',num2str(px(i)))
end
...
for i = 1:nparams-1
  for l = (i-1)*nparams+1:i*(nparams-1)
    ...
    j = l-(i-1)*(nparams-1)+1;
    plot(x(j,:),x(i,:),'k.')
    xlabel([append('$',names(j),'$')],'interpreter','latex')
    ylabel([append('$',names(i),'$')],'interpreter','latex')
  end
end