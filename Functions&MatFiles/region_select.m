function[binaryImage, nregions] = region_select(b,nm,fignum)
% This function creates masks for the different regions of an image for use
%   in inverse problems. This is compatible only with R2019a and later. Use
%   region_select_old for older versions.
...
while answer_draw == 1
    ...
    '\nPress enter after drawing each part of the region.'...
    '\nDouble click on edge to add a new adjustable point.'...
    '\nYou can adjust any region at any time.'...
    ...
    H{1} = cell(1,1);
    H{1}{1} = drawfreehand;
    if isvalid(H{1}{1}) && size(H{1}{1}.Position,1) > 10
        invalid_flag = 0;
        pause;
        H{nregions}{regparts}.Color = 'red';
        % less than 10 pixels is considered empty mask
        while size(H{nregions}{regparts}.Position,1) > 10
            %totMask = totMask | BWmask; % add mask to global mask
             % ask user for another mask
            regparts = regparts + 1;
            H{nregions}{regparts} = drawfreehand;
            pause;
            H{nregions}{regparts}.Color = 'red';
        end
        ...
        % Handle response
            switch answer_more
                case 'Yes'
                    ...
                    regparts(nregions) = 1;
                    H{nregions}{regparts(nregions)} = drawfreehand;
                    pause;
                    H{nregions}{regparts(nregions)}.Color = reg_color;
                    % less than 10 pixels is considered empty mask
                    while size(H{nregions}{regparts(nregions)}.Position,1) > 10
...
if invalid_flag == 1
    ...
    for i = 1:nregions
        binaryImage{i} = false(sz1,sz2);
        for j = 1:regparts(i)
            BWmask = H{i}{j}.createMask;
            binaryImage{i} = binaryImage{i} | BWmask;
        end
...
if nm(1)>szb1 || nm(2)>szb2
    ...
    %nregions = nregions+1;
%     binaryImage{nregions+1} = true(nm(1),nm(2));
%     binaryImage{nregions+1}((nm(1)-szb1)/2+1:3*(256-128)/2,(nm(1)-szb1)/2+1:3*(256-128)/2)...
%         = true(szb1,szb2);
end
