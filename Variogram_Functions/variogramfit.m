...
if params.plotit
    ...
    axis([0 max(h) 0 max(gammaexp)+.2*max(gammaexp)])
    xlabel('Lag Distance (r)', 'FontSize', 14)
    ylabel('Semivariance (\gamma(r))', 'FontSize', 14)
    leg = legend('Empirical Semivariogram','Matern Model Fit','Location','northwest');
    if params.model == 'matern'
        leg = legend('Empirical Semivariogram',...
            ['Matern Model Fit (\nu = ' num2str(params.nu)...
            ', \kappa = ' num2str(round(1/a,3)) ')'],'Location','northwest');
    end
    set(leg, 'FontSize', 12);
    hold off
end
