function [l1, l2, theta, nu] = variogram_params_aniso_dists(xycoords, vec, dists, nums)
...
% 
% This function should only be used once variogram_params_aniso has already
%   been run so dists and nums have been calculated and saved.
% 
% The inputs are:
...
% dists = the array of all distances between spatial coordinates
% nums = the number of values at each distance in dists
...
disp(' *** Updating semivariogram estimates *** ')
maxdist = sqrt(sum((max(xycoords)-min(xycoords)).^2));
S_dir = variogram_dists(xycoords,vec,dists,nums,'nrbins',25,'maxdist',maxdist/2,...
