function [a,c,n,S] = variogramfit_dir(h,gammaexp,a0,c0,numobs,theta,varargin)
...
%               D'Errico, File ID: #on the FEX). The advantage in 
...
switch lower(params.solver)
    case {'fminsearchbnd'};
        ...
        % upper bounds
        if nugget;
            ub = [inf 2*max(gammaexp) 2*max(gammaexp)]; %
        else
            ub = [inf 2*max(gammaexp)];
        end
end
...
if params.plotit
    ...
    xlabel('Lag Distance (r)', 'FontSize', 14)
    ylabel('Semivariance (\gamma(r))', 'FontSize', 14)
    title(['\theta = ' num2str(theta/pi*180)...
        ', \kappa = ' num2str(round(1/a,3))])
    %leg = legend('Location','northwest','Empirical Semivariogram','Matern Model Fit');
    %if params.model == 'matern'
    %    leg = legend('Location','northwest','Empirical Semivariogram',...
    %        ['Matern Model Fit (\nu = ' num2str(params.nu)...
    %        ', \kappa = ' num2str(round(1/a,3)) ')']);
    %end
    %set(leg, 'FontSize', 12);
